...
$sql_res = "select ch.cta_b, ch.cheque, ch.fecha, ben.benef, ch.monto, ch.proy, ch.cta, ch.responsable, ch.estatus, ch.fecha_c from tbl_cheques ch, tbl_benef ben, tbl_proyectos p where ch.benef_id=ben.benef_id and ch.proy=p.proy and ch.estatus in ('Facturas','Comprobado') and (oficio='' or oficio is NULL) and p.ures $seleccion and ch.responsable='$res' order by ch.cta_b, ch.cheque";
$qry_res = mysql_query($sql_res);

echo ("<table id='info' align='center' border='1'><thead><tr><th colspan='10'>Cheques en Proceso de Comprobación</th></tr><tr> <th>Cta Bancaria</th> <th>Cheque</th> <th>Fecha</th> <th>Beneficiario</th> <th>Monto</th> <th>Proyecto</th> <th>Cuenta</th> <th>Responsable</th> <th>Estatus</th> <th>Fecha de Recibido</th> </tr></thead><tbody>");
$renglon='non';//cambio de color en renglones
while ($arr_res = mysql_fetch_array($qry_res)){
	echo ("<tr id='$renglon'> <td>".$arr_res['cta_b']."</td> <td>".$arr_res['cheque']."</td> <td>".$arr_res['fecha']."</td> <td id='benef'>".utf8_decode($arr_res['benef'])."</td> <td id='monto'>".number_format($arr_res['monto'],2)."</td> <td>".$arr_res['proy']."</td> <td>".$arr_res['cta']."</td> <td>".$arr_res['responsable']."</td> <td>".$arr_res['estatus']."</td> <td>".$arr_res['fecha_c']."</td> </tr>");
