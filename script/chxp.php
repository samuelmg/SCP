...
function chxp($proy){
include ("fecha.php");
$sql_ch = "select ch.cta_b, ch.cheque, ch.fecha, ben.benef, ch.monto, ch.proy, ch.cta, ch.cmt, ch.estatus from tbl_cheques ch, tbl_benef ben where ch.benef_id = ben.benef_id and ch.proy = $proy order by ch.cta_b, ch.cheque";
$qry_ch = mysql_query($sql_ch);

echo ("<table align='center' id='info'><thead><tr> <th>Cta Bancaria</th> <th>Cheque</th> <th>Fecha</th> <th>Beneficiario</th> <th>Monto</th> <th>Proyecto</th> <th>Cuenta</th> <th>Comentarios</th> <th>Estatus</th> </tr></thead><tbody>");
$renglon='non';//cambio de color en renglones
while ($arr_ch = mysql_fetch_array($qry_ch)){
	$fecha=$arr_ch['fecha'];
	fecha_info(&$fecha);
	echo ("<tr id='$renglon'> <td>".$arr_ch['cta_b']."</td> <td>".$arr_ch['cheque']."</td> <td>".$fecha."</td> <td id='benef'>".utf8_decode($arr_ch['benef'])."</td> <td id='monto'>".number_format($arr_ch['monto'],2)."</td> <td>".$arr_ch['proy']."</td> <td>".$arr_ch['cta']."</td> <td id='benef'>".$arr_ch['cmt']."</td> <td>".$arr_ch['estatus']."</td> </tr>");
